...
export default function BusinessCenterPortal() {
  ...
  const handleFlutterwaveCheckout = async (e) => {
    ...
    const cleanRegNumber = String(formData.regNumber || '').trim();
        ...
        if (data.status === "successful" || data.status === "completed") {
          try {
            ...
            if (error) {
              ...
              alert(`Payment successful, but failed to log. Contact management with reference: ${uniqueTxRef}. Error: ${error.message}`);
            } else {
